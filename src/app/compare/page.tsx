...
function CompareContent() {
  ...
  const { data: queryProviders = [], isLoading, isError, error } = useQuery({
    queryKey: ['providers', 'compare', idsParam],
    queryFn: async () => {
      ...
      if (dbIds.length > 0) {
        // Enforce approved-only status filter
        ...
        if (data) {
          ...
          const typedDbProviders = data as DbProvider[];

          // Refactor N+1 review queries into one batch fetch
          ...
          // Fetch services for all matching dbIds to resolve price level dynamically
          ...
          if (allServices) {
            allServices.forEach((s: { provider_id: string; price: number }) => {
              ...
              servicesMap[s.provider_id].push(s.price);
            });
          }

          dbResults = typedDbProviders.map((p) => {
            const ratings = reviewsMap[p.id] || [];
            const count = ratings.length;
            
            // Average rating returns a neutral "No rating" (or -1 representation) instead of 5.0 when count is zero
            const avg = count > 0 
              ? Number((ratings.reduce((acc, curr) => acc + curr, 0) / count).toFixed(2)) 
              : -1; // -1 represents "No rating"

            // Derive price level dynamically from actual services.
            // If the provider has services, map the average price to $, $$, or $$$ tiers.
            const prices = servicesMap[p.id] || [];
  ...
  // If the query fails, fallback to STATIC_PROVIDERS and log error
  let displayProviders = queryProviders;
  if (isError) {
    console.error("Failed to load compared providers:", error);
    displayProviders = STATIC_PROVIDERS.filter(p => ids.includes(p.id));
  }
                    ...
                    <div className="mb-4">
                      ...
                      <div className="flex items-center gap-1.5 text-sm text-slate-600 dark:text-slate-350">
                        ...
                        <span className="font-bold text-on-surface">
                          {p.avg_rating === -1 ? 'No rating' : p.avg_rating}
                        </span>
